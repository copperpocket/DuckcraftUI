...
-- ============================================================================
-- EDIT BOX (single-line text input, db-bound, commits on Enter)
-- ============================================================================

function Controls:AddEditBox(parent, opts)
    opts = opts or {}
    local eb = AceGUI:Create("EditBox")
    eb:SetLabel(opts.label or "")
    if opts.width then eb:SetWidth(opts.width) else eb:SetFullWidth(true) end
    ...
    eb:SetText(val or opts.default or "")

    if opts.disabled then
        if type(opts.disabled) == "function" then
            eb:SetDisabled(opts.disabled())
        else
            eb:SetDisabled(opts.disabled)
        end
    end

    eb:SetCallback("OnEnterPressed", function(w, _, text)
        if opts.setFunc then
            opts.setFunc(text)
        elseif opts.dbPath then
            self:SetDBValue(opts.dbPath, text)
        end
    ...
    if opts.desc then
        eb:SetCallback("OnEnter", function(w)
