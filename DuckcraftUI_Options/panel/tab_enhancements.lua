...
local function BuildEnhancementsTab(scroll)
    ...
    local buffVisibility = C:AddSection(scroll, LO["Buff Frame Visibility"])

    C:AddDescription(
        buffVisibility,
        LO["Show the buff frame only when selected conditions are active. Manual buff collapse remains respected."]
    )

    local function RefreshBuffVisibility()
        if addon.RefreshBuffVisibility then
            addon.RefreshBuffVisibility()
        end
    end

    C:AddToggle(buffVisibility, {
        label = LO["Show on Hover"],
        dbPath = "buffs.visibility.show_on_hover",
        callback = RefreshBuffVisibility,
    })

    C:AddToggle(buffVisibility, {
        label = LO["Show in Combat"],
        dbPath = "buffs.visibility.show_in_combat",
        callback = RefreshBuffVisibility,
    })

    C:AddToggle(buffVisibility, {
        label = LO["Show with Target"],
        dbPath = "buffs.visibility.show_with_target",
        callback = RefreshBuffVisibility,
    })

    C:AddToggle(buffVisibility, {
        label = LO["Show When Health Is Not Full"],
        dbPath = "buffs.visibility.show_on_health",
        callback = RefreshBuffVisibility,
    })

    C:AddToggle(buffVisibility, {
        label = LO["Show When Power Is Not Full"],
        dbPath = "buffs.visibility.show_on_power",
        callback = RefreshBuffVisibility,
    })

    C:AddSlider(buffVisibility, {
        label = LO["Fade Duration"],
        desc = LO["Time in seconds used to fade the buff frame in or out. Set to 0 for instant visibility changes."],
        dbPath = "buffs.visibility.fade_duration",
        min = 0,
        max = 3,
        step = 0.05,
        width = 200,
