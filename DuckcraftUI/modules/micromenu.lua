...
local function UpdateCharacterPortraitVisibility()
    if MicroButtonPortrait then
        if addon and addon.db and addon.db.profile and addon.db.profile.micromenu and
            ...
            -- HandleDuckcraftUIState manage it to avoid race conditions.
        end
    end
    ...
    if charBtn and not useGrayscale and charBtn.DuckcraftUICharHighlight then
        SetPortraitTexture(charBtn.DuckcraftUICharHighlight, "player")
        ...
        charBtn.DuckcraftUICharHighlight:SetTexCoord(MicroButtonPortrait:GetTexCoord())
        charBtn.DuckcraftUICharHighlight:SetBlendMode('ADD')
        charBtn.DuckcraftUICharHighlight:SetAlpha(0.5)
        charBtn.DuckcraftUICharHighlight:SetAllPoints(MicroButtonPortrait)
    end
    if charBtn and charBtn.DuckcraftUIBackground then
        local microTexture = 'Interface\\AddOns\\DuckcraftUI\\Textures\\Micromenu\\uimicromenu2x'

        -- Rehydrate texture data in case a late UI pass stripped it.
        charBtn.DuckcraftUIBackground:SetTexture(microTexture)
        charBtn.DuckcraftUIBackground:SetTexCoord(0.0654297, 0.12793, 0.330078, 0.490234)
        if charBtn.DuckcraftUIBackgroundPushed then
            charBtn.DuckcraftUIBackgroundPushed:SetTexture(microTexture)
            charBtn.DuckcraftUIBackgroundPushed:SetTexCoord(0.0654297, 0.12793, 0.494141, 0.654297)
        end

        if useGrayscale then
            charBtn.DuckcraftUIBackground:Hide()
            if charBtn.DuckcraftUIBackgroundPushed then
                charBtn.DuckcraftUIBackgroundPushed:Hide()
            end
        else
            if charBtn.duckcraftUIState and charBtn.duckcraftUIState.pushed and charBtn.DuckcraftUIBackgroundPushed then
                charBtn.DuckcraftUIBackground:Hide()
                charBtn.DuckcraftUIBackgroundPushed:Show()
            else
                charBtn.DuckcraftUIBackground:Show()
                if charBtn.DuckcraftUIBackgroundPushed then
                    charBtn.DuckcraftUIBackgroundPushed:Hide()
                end
            end
    ...
    elseif charBtn and not (addon and addon.db and addon.db.profile and addon.db.profile.micromenu and addon.db.profile.micromenu.grayscale_icons) then
        ...
        charBtn.DuckcraftUIBackground = bg
        ...
        bgPushed:Hide()
        charBtn.DuckcraftUIBackgroundPushed = bgPushed
    ...
    for _, button in pairs(MICRO_BUTTONS) do
        if button then
            ...
            if original then
                ...
                -- Clean up DuckcraftUI custom textures
                if button.DuckcraftUIBackground then
                    button.DuckcraftUIBackground:Hide()
                    button.DuckcraftUIBackground = nil
                end
                if button.DuckcraftUIBackgroundPushed then
                    button.DuckcraftUIBackgroundPushed:Hide()
                    button.DuckcraftUIBackgroundPushed = nil
                end

                button.duckcraftUIState = nil
                button.duckcraftUITimer = nil
                button.duckcraftUILastState = nil
                button.HandleDuckcraftUIState = nil
            end
        end
    end
...
local function ApplyMicromenuSystem()
    ...
    local function SetupPVPButton(button)
        ...
        -- we create our own ARTWORK texture (DuckcraftUIPVPIcon) that we control
        -- exclusively, just like Character uses MicroButtonPortrait.
        local microTexture = 'Interface\\AddOns\\DuckcraftUI\\Textures\\Micromenu\\micropvp'
        local englishFaction = UnitFactionGroup('player')
        local backgroundTexture = 'Interface\\AddOns\\DuckcraftUI\\Textures\\Micromenu\\uimicromenu2x'
        ...
        if not button.DuckcraftUIPVPIcon then
            local icon = button:CreateTexture(nil, 'ARTWORK')
            button.DuckcraftUIPVPIcon = icon
        end

        local icon = button.DuckcraftUIPVPIcon
        ...
        if not button.DuckcraftUIBackground then
            local bg = button:CreateTexture(nil, 'BACKGROUND')
            bg:SetTexture(backgroundTexture)
            bg:SetSize(sizeX, sizeY + 1)
            bg:SetTexCoord(0.0654297, 0.12793, 0.330078, 0.490234)
            bg:SetPoint('CENTER', dx, dy)
            button.DuckcraftUIBackground = bg
            ...
            bgPushed:SetPoint('CENTER', dx + offX, dy + offY)
            bgPushed:Hide()
            button.DuckcraftUIBackgroundPushed = bgPushed
        else
            button.DuckcraftUIBackground:SetTexture(backgroundTexture)
            button.DuckcraftUIBackground:SetTexCoord(0.0654297, 0.12793, 0.330078, 0.490234)
            button.DuckcraftUIBackground:ClearAllPoints()
            button.DuckcraftUIBackground:SetPoint('CENTER', dx, dy)
            button.DuckcraftUIBackground:SetSize(sizeX, sizeY + 1)

            if button.DuckcraftUIBackgroundPushed then
                button.DuckcraftUIBackgroundPushed:SetTexture(backgroundTexture)
                button.DuckcraftUIBackgroundPushed:SetTexCoord(0.0654297, 0.12793, 0.494141, 0.654297)
                button.DuckcraftUIBackgroundPushed:ClearAllPoints()
                button.DuckcraftUIBackgroundPushed:SetPoint('CENTER', dx + offX, dy + offY)
                button.DuckcraftUIBackgroundPushed:SetSize(sizeX, sizeY + 1)
            end
        end

        -- ---- State tracking ----
        button.duckcraftUIState = button.duckcraftUIState or {}
        button.duckcraftUIState.pushed = IsSpecialMicroButtonActive(button, "PVP")
        button.duckcraftUILastState = button.duckcraftUIState.pushed
        button.duckcraftUITimer = button.duckcraftUITimer or 0

        -- ---- State handler: only manipulates DuckcraftUIPVPIcon ----
        button.HandleDuckcraftUIState = function()
            local pvpIcon = button.DuckcraftUIPVPIcon
            local state = button.duckcraftUIState
            ...
            if state and state.pushed then
                ...
                if button.DuckcraftUIBackgroundPushed then
                    button.DuckcraftUIBackgroundPushed:Show()
                end
                ...
                if button.DuckcraftUIBackground then
                    button.DuckcraftUIBackground:Show()
                end
                if button.DuckcraftUIBackgroundPushed then
                    button.DuckcraftUIBackgroundPushed:Hide()
                end
        ...
        button:SetScript('OnUpdate', function(self, elapsed)
            self.duckcraftUITimer = (self.duckcraftUITimer or 0) + elapsed
            if self.duckcraftUITimer >= 0.1 then
                self.duckcraftUITimer = 0
                local currentState = IsSpecialMicroButtonActive(self, "PVP")
                if currentState ~= self.duckcraftUILastState then
                    self.duckcraftUILastState = currentState
                    if self.duckcraftUIState then
                        self.duckcraftUIState.pushed = currentState
                    end
                    if self.HandleDuckcraftUIState then
                        self.HandleDuckcraftUIState()
                    end
                end
            end
        end)
        ...
        if not button.DuckcraftUIStateHooks then
            button:HookScript('OnMouseDown', function(self)
                if self.duckcraftUIState then
                    self.duckcraftUIState.pushed = true
                end
                if self.HandleDuckcraftUIState then
                    self.HandleDuckcraftUIState()
                end
            ...
            button.DuckcraftUIStateHooks = true
        end

        -- Apply initial state
        button.HandleDuckcraftUIState()
    end
    ...
    local function SetupCharacterButton(button)
        ...
        -- own DuckcraftUIBackground/BackgroundPushed instead.
        ...
        -- identical position/size — same technique as DuckcraftUIPortraitDim.
        local function RefreshCharHighlight(btn)
            local hl = btn.DuckcraftUICharHighlight
        ...
        if not button.DuckcraftUICharHighlight then
            local hl = button:CreateTexture(nil, 'OVERLAY')
            hl:SetAllPoints(portraitTexture)
            hl:Hide()
            button.DuckcraftUICharHighlight = hl

            button:HookScript('OnEnter', function(self)
                if self.DuckcraftUICharHighlight then
                    RefreshCharHighlight(self)
                    self.DuckcraftUICharHighlight:Show()
                end
            end)
            button:HookScript('OnLeave', function(self)
                if self.DuckcraftUICharHighlight then
                    self.DuckcraftUICharHighlight:Hide()
                end
            end)
        end
        ...
        if not charPushHooksRegistered then
            hooksecurefunc('CharacterMicroButton_SetPushed', function()
                ...
                local pt = button:GetPushedTexture()
                if pt then pt:SetTexture(nil) end
                local hl = button.DuckcraftUICharHighlight
        ...
        button.DuckcraftUICharHighlight:Hide()

        -- Keep highlight in sync when portrait model updates (first login).
        if not button.DuckcraftUIPortraitEventRegistered then
            ...
            button.DuckcraftUIPortraitEventRegistered = true
        end
        ...
        if not button.DuckcraftUIBackground then
            ...
            button.duckcraftUIState = {
                pushed = false
            }
            button.duckcraftUITimer = 0
            button.duckcraftUILastState = button.duckcraftUIState.pushed
            ...
            button.DuckcraftUIBackground:SetTexture(microTexture)
            button.DuckcraftUIBackground:SetTexCoord(0.0654297, 0.12793, 0.330078, 0.490234)
            button.DuckcraftUIBackground:ClearAllPoints()
            button.DuckcraftUIBackground:SetPoint('CENTER', dx, dy)
            button.DuckcraftUIBackground:SetSize(sizeX, sizeY + 1)
            ...
            if button.DuckcraftUIBackgroundPushed then
                button.DuckcraftUIBackgroundPushed:SetTexture(microTexture)
                button.DuckcraftUIBackgroundPushed:SetTexCoord(0.0654297, 0.12793, 0.494141, 0.654297)
                button.DuckcraftUIBackgroundPushed:ClearAllPoints()
                button.DuckcraftUIBackgroundPushed:SetPoint('CENTER', dx + offX, dy + offY)
                button.DuckcraftUIBackgroundPushed:SetSize(sizeX, sizeY + 1)
            ...
            button.duckcraftUIState = button.duckcraftUIState or { pushed = false }

            -- Delegate to the existing handler for consistent state application
            if button.HandleDuckcraftUIState then
                button.HandleDuckcraftUIState()
            end
        end
    end
    ...
    local function setupMicroButtons(xOffset)
        ...
        for _, button in pairs(MICRO_BUTTONS) do
            if button then
                ...
                if shouldUseGrayscale then
                    ...
                    if button.DuckcraftUIHover then
                        button.DuckcraftUIHover:Hide()
                    end
                ...
                elseif isCharacterButton then
                    ...
                    if not button.DuckcraftUIBackground then
                        ...
                        button.duckcraftUITimer = 0
                        button.duckcraftUILastState = false
                        ...
                        button:SetScript('OnUpdate', function(self, elapsed)
                            ...
                            if not self.duckcraftUITimer then
                                self.duckcraftUITimer = 0
                            end

                            self.duckcraftUITimer = self.duckcraftUITimer + elapsed
                            if self.duckcraftUITimer >= 0.1 then
                                self.duckcraftUITimer = 0
                        ...
                        button:HookScript("OnMouseDown", function(self)
                            if not self.duckcraftUIState then return end
                            -- Only act on push direction (not already pushed)
                            if not self.duckcraftUIState.pushed then
                                self.duckcraftUIState.pushed = true
                                self.duckcraftUILastState = true
                                if self.HandleDuckcraftUIState then
                                    self.HandleDuckcraftUIState()
                                end
                            end
                        ...
                        if not button.DuckcraftUISetButtonStateHooked then
                            hooksecurefunc(button, "SetButtonState", function(self, state)
                                if state ~= "PUSHED" then
                                    -- Only act on unpush direction
                                    if self.duckcraftUIState and self.duckcraftUIState.pushed then
                                        self.duckcraftUIState.pushed = false
                                        self.duckcraftUILastState = false
                                        if self.HandleDuckcraftUIState then
                                            self.HandleDuckcraftUIState()
                                        end
                                    end
                            ...
                            button.DuckcraftUISetButtonStateHooked = true
                        end
                    else
                        ...
                        button.DuckcraftUIBackgroundPushed:SetPoint('CENTER', dx + offX, dy + offY)
                        button.DuckcraftUIBackgroundPushed:SetSize(sizeX, sizeY + 1)
                        if button.duckcraftUIState and button.duckcraftUIState.pushed then
                            button.DuckcraftUIBackground:Hide()
                            button.DuckcraftUIBackgroundPushed:Show()
                        else
                            button.DuckcraftUIBackgroundPushed:Hide()
        ...
        if showLatency and HelpMicroButton then
            if not MicromenuModule.frames.latencyIndicator then
                local latencyBar = CreateFrame("StatusBar", "DuckcraftUIPerformanceBar", HelpMicroButton)
    ...
    local microMenuVisibilityState = {
        hovered = false,
    ...
    local function GetMicroMenuVisibilityConfig()
        local microConfig = addon.db
            and addon.db.profile
            and addon.db.profile.micromenu

        return microConfig and microConfig.visibility
    end

    local function GetMicroMenuEditorFrame()
        local info = addon:GetEditableFrameInfo("micromenu")
        return info and info.frame
    end

    local function IsCursorOverMicroMenu()
        local frame = GetMicroMenuEditorFrame()
    ...
    local function ShouldShowMicroMenu()
        ...
        if config.show_in_combat and microMenuVisibilityState.inCombat then
            return true
        end
    ...
    local function GetMicroMenuFadeDuration()
        local config = GetMicroMenuVisibilityConfig()
        return tonumber(config and config.fade_duration) or 0
    end

    local function ApplyMicroMenuAlpha(alpha)
        local menu = _G.pUiMicroMenu
        local editorFrame = GetMicroMenuEditorFrame()

        -- Keep the editor frame available for positioning and hover detection.
        if editorFrame then
            editorFrame:Show()
        end
        ...
        -- Respect the existing vehicle state-driver behavior.
        -- Do not Show() the menu while the vehicle option should hide it.
        if addon.db
            and addon.db.profile
            and addon.db.profile.micromenu
            and addon.db.profile.micromenu.hide_on_vehicle
    ...
    local function StartMicroMenuFade(shouldShow)
        local state = microMenuVisibilityState
        local targetAlpha = shouldShow and 1 or 0
        local duration = GetMicroMenuFadeDuration()
        ...
        if state.fading then
            ...
            currentAlpha = menu:GetAlpha() or 1
        end

        currentAlpha = math.max(0, math.min(1, currentAlpha))

        -- A duration of zero preserves instant visibility behavior.
        if duration <= 0
            ...
            ApplyMicroMenuAlpha(targetAlpha)
            return
        end
        ...
        -- Apply immediately so a reversing fade starts from its current alpha.
    ...
    function addon.RefreshMicromenuVisibility()
        ...
        RefreshMicroMenuVisibilityInternal()
    end

    -- Fade animation
    local microMenuFadeFrame = CreateFrame("Frame")
    microMenuFadeFrame:Show()

    microMenuFadeFrame:SetScript("OnUpdate", function(_, elapsed)
        local state = microMenuVisibilityState
        ...
        ApplyMicroMenuAlpha(state.alpha)
    end)

    -- Hover polling. This continues to work while the actual menu is alpha-hidden.
    local microMenuHoverFrame = CreateFrame("Frame")
    local microMenuHoverElapsed = 0

    microMenuHoverFrame:SetScript("OnUpdate", function(_, elapsed)
        microMenuHoverElapsed = microMenuHoverElapsed + elapsed

        if microMenuHoverElapsed < 0.05 then
            return
        end

        microMenuHoverElapsed = 0

        local config = GetMicroMenuVisibilityConfig()

        if not config or not config.show_on_hover then
            if microMenuVisibilityState.hovered then
                microMenuVisibilityState.hovered = false
                RefreshMicroMenuVisibilityInternal()
            end
        ...
        local hovered = IsCursorOverMicroMenu()

        if hovered ~= microMenuVisibilityState.hovered then
            microMenuVisibilityState.hovered = hovered
            RefreshMicroMenuVisibilityInternal()
        end
    end)

    -- Combat, target, health, power, and vehicle refresh events.
    local microMenuVisibilityEvents = CreateFrame("Frame")

    microMenuVisibilityEvents:RegisterEvent("PLAYER_TARGET_CHANGED")
    microMenuVisibilityEvents:RegisterEvent("PLAYER_REGEN_DISABLED")
    microMenuVisibilityEvents:RegisterEvent("PLAYER_REGEN_ENABLED")
    microMenuVisibilityEvents:RegisterEvent("UNIT_ENTERED_VEHICLE")
    microMenuVisibilityEvents:RegisterEvent("UNIT_EXITED_VEHICLE")
    microMenuVisibilityEvents:RegisterEvent("UNIT_HEALTH")
    microMenuVisibilityEvents:RegisterEvent("UNIT_MAXHEALTH")
    microMenuVisibilityEvents:RegisterEvent("UNIT_HEALTH_FREQUENT")
    microMenuVisibilityEvents:RegisterEvent("UNIT_MANA")
    microMenuVisibilityEvents:RegisterEvent("UNIT_MAXMANA")
    microMenuVisibilityEvents:RegisterEvent("UNIT_RAGE")
    microMenuVisibilityEvents:RegisterEvent("UNIT_FOCUS")
    microMenuVisibilityEvents:RegisterEvent("UNIT_ENERGY")
    microMenuVisibilityEvents:RegisterEvent("UNIT_RUNIC_POWER")
    microMenuVisibilityEvents:RegisterEvent("UNIT_POWER_UPDATE")
    microMenuVisibilityEvents:RegisterEvent("UNIT_MAXPOWER")
    microMenuVisibilityEvents:RegisterEvent("UNIT_DISPLAYPOWER")

    microMenuVisibilityEvents:SetScript("OnEvent", function(_, event, unit)
        if event:sub(1, 5) == "UNIT_" and unit ~= "player" then
            return
        end

        if event == "PLAYER_REGEN_DISABLED"
            or event == "UNIT_ENTERED_VEHICLE" then
            microMenuVisibilityState.inCombat = true
        elseif event == "PLAYER_REGEN_ENABLED"
            or event == "UNIT_EXITED_VEHICLE" then
            microMenuVisibilityState.inCombat = false
        end
    ...
    local function GetBagBarVisibilityConfig()
        local bags = addon.db
            and addon.db.profile
            and addon.db.profile.bags

        return bags and bags.visibility
    end

    local function GetBagBarFrame()
        local info = addon:GetEditableFrameInfo("bagsbar")
        return info and info.frame
    end

    local function IsCursorOverBagBar()
        local frame = GetBagBarFrame()
        if not frame then
            return false
        end
    ...
    local function ShouldShowBagBar()
        ...
        if config.show_in_combat and bagBarVisibilityState.inCombat then
            return true
        end
        ...
        if config.show_on_power then
            local power = UnitPower and UnitPower("player")
            local maxPower = UnitPowerMax and UnitPowerMax("player")
    ...
    local function ApplyBagBarAlpha(alpha)
        local container = GetBagBarFrame()
        ...
        if addon.pUiArrowManager then
            addon.pUiArrowManager:SetAlpha(alpha)
        end
    end

    local function GetBagBarFadeDuration()
        local config = GetBagBarVisibilityConfig()
        return tonumber(config and config.fade_duration) or 0
    end

    local function StartBagBarFade(shouldShow)
        local state = bagBarVisibilityState
        local targetAlpha = shouldShow and 1 or 0
        local duration = GetBagBarFadeDuration()

        local currentAlpha
        if state.fading then
            currentAlpha = state.alpha
        else
            currentAlpha = MainMenuBarBackpackButton
                and MainMenuBarBackpackButton:GetAlpha()
                or 1
        end
    ...
    function addon.RefreshBagBarVisibility()
        ...
        RefreshBagBarVisibilityInternal()
    end

    local bagBarFadeFrame = CreateFrame("Frame")
    bagBarFadeFrame:Show()

    bagBarFadeFrame:SetScript("OnUpdate", function(_, elapsed)
        local state = bagBarVisibilityState
        ...
        ApplyBagBarAlpha(state.alpha)
    end)

    local bagBarHoverFrame = CreateFrame("Frame")
    local bagBarHoverElapsed = 0

    bagBarHoverFrame:SetScript("OnUpdate", function(_, elapsed)
        bagBarHoverElapsed = bagBarHoverElapsed + elapsed

        if bagBarHoverElapsed < 0.05 then
            return
        end

        bagBarHoverElapsed = 0

        local config = GetBagBarVisibilityConfig()

        if not config or not config.show_on_hover then
            if bagBarVisibilityState.hovered then
                bagBarVisibilityState.hovered = false
                RefreshBagBarVisibilityInternal()
            end
            return
        end

        local hovered = IsCursorOverBagBar()

        if hovered ~= bagBarVisibilityState.hovered then
            bagBarVisibilityState.hovered = hovered
            RefreshBagBarVisibilityInternal()
        end
    end)

    local bagBarEventFrame = CreateFrame("Frame")

    bagBarEventFrame:RegisterEvent("PLAYER_TARGET_CHANGED")
    bagBarEventFrame:RegisterEvent("PLAYER_REGEN_DISABLED")
    bagBarEventFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
    bagBarEventFrame:RegisterEvent("UNIT_HEALTH")
    bagBarEventFrame:RegisterEvent("UNIT_MAXHEALTH")
    bagBarEventFrame:RegisterEvent("UNIT_HEALTH_FREQUENT")
    bagBarEventFrame:RegisterEvent("UNIT_MANA")
    bagBarEventFrame:RegisterEvent("UNIT_MAXMANA")
    bagBarEventFrame:RegisterEvent("UNIT_RAGE")
    bagBarEventFrame:RegisterEvent("UNIT_FOCUS")
    bagBarEventFrame:RegisterEvent("UNIT_ENERGY")
    bagBarEventFrame:RegisterEvent("UNIT_RUNIC_POWER")
    bagBarEventFrame:RegisterEvent("UNIT_POWER_UPDATE")
    bagBarEventFrame:RegisterEvent("UNIT_MAXPOWER")
    bagBarEventFrame:RegisterEvent("UNIT_DISPLAYPOWER")

    bagBarEventFrame:SetScript("OnEvent", function(_, event, unit)
        if event:sub(1, 5) == "UNIT_" and unit ~= "player" then
            return
        end

        if event == "PLAYER_REGEN_DISABLED" then
            bagBarVisibilityState.inCombat = true
        elseif event == "PLAYER_REGEN_ENABLED" then
            bagBarVisibilityState.inCombat = false
        end
    ...
    if addon.RefreshBagBarVisibility then
        addon.RefreshBagBarVisibility()
    end
