...
function MinimapModule:InitializeMinimapSystem()
    ...
    -- Warm the transparent blip through the REAL SetBlipTexture path once, so
    -- the first in-game blank (during the initial fade-out) applies reliably.
    -- Without this, the first SetBlipTexture(blank) of the session doesn't fully
    -- take and real quest blips show once on the first fade-in. UpdateSettings()
...
local function GetMinimapVisibilityConfig()
    local config = addon.db
        and addon.db.profile
        and addon.db.profile.minimap

    return config and config.visibility
end

local function GetMinimapHoverFrame()
    return MinimapModule.minimapFrame
end

local function IsCursorOverMinimap()
    local frame = GetMinimapHoverFrame()
...
local function ShouldShowMinimap()
    ...
    if config.show_in_combat and minimapVisibilityState.inCombat then
        return true
    end
...
local function GetMinimapFadeDuration()
    local config = GetMinimapVisibilityConfig()
...
local function ApplyAlphaToMinimapTree(frame, alpha, visited)
    ...
    -- Set the frame alpha explicitly. Some child frames override their
    -- parent's effective alpha with their own alpha value.
    if frame.SetAlpha then
        frame:SetAlpha(alpha)
    end

    -- POI arrows, quest markers, corpse arrows, and similar elements may be
    -- direct texture regions rather than child frames.
    ...
    -- Recurse through child frames, including addon minimap buttons.
    if frame.GetNumChildren then
        for i = 1, frame:GetNumChildren() do
            local child = select(i, frame:GetChildren())
            ApplyAlphaToMinimapTree(child, alpha, visited)
        end
    end
end
...
-- frames, so alpha can't tween them — we can only hide/show them. Arrows accept
...
local function ApplyMinimapAlpha(alpha)
    ...
    if isHidden then
        ...
        if MinimapModule.borderFrame then MinimapModule.borderFrame:Hide() end

        if MiniMapLFGFrame then
            MinimapModule._lfgWasVisible = MiniMapLFGFrame:IsShown()
            MiniMapLFGFrame:Hide()
        end
        if MiniMapBattlefieldFrame then
            MinimapModule._battlefieldWasVisible = MiniMapBattlefieldFrame:IsShown()
            MiniMapBattlefieldFrame:Hide()
        end
        if DurabilityFrame then
            MinimapModule._durabilityWasVisible = DurabilityFrame:IsShown()
            DurabilityFrame:Hide()
        end

        MinimapModule._minimapVisibilityHidden = true
        return
    end

    -- Restore roots and fade them together.
    -- CRITICAL: set alpha on Minimap itself, not just the cluster —
    -- POI arrows and quest blips are engine-rendered and honor only
    -- the Minimap frame's own alpha, not a cascaded cluster alpha.
    if MinimapCluster then
        MinimapCluster:Show()
    ...
    if MinimapBackdrop then
        ...
        MinimapBackdrop:SetAlpha(alpha)
    end
    if MinimapModule.borderFrame then
        MinimapModule.borderFrame:Show()
        MinimapModule.borderFrame:SetAlpha(alpha)
    end

    if MiniMapLFGFrame then
        if MinimapModule._lfgWasVisible then
            MiniMapLFGFrame:Show(); MiniMapLFGFrame:SetAlpha(alpha)
        else
            MiniMapLFGFrame:Hide()
        end
    end
    if MiniMapBattlefieldFrame then
        if MinimapModule._battlefieldWasVisible then
            MiniMapBattlefieldFrame:Show(); MiniMapBattlefieldFrame:SetAlpha(alpha)
        else
            MiniMapBattlefieldFrame:Hide()
        end
    end
    if DurabilityFrame then
        if MinimapModule._durabilityWasVisible then
            DurabilityFrame:Show(); DurabilityFrame:SetAlpha(alpha)
        else
            DurabilityFrame:Hide()
        end
    end
...
local function RefreshConditionalMinimapFrames()
    if not MinimapModule._minimapVisibilityHidden then
        return
    end

    if MiniMapLFGFrame and MiniMapLFGFrame:IsShown() then
        MinimapModule._lfgWasVisible = true
        MiniMapLFGFrame:Hide()
    end

    if MiniMapBattlefieldFrame and MiniMapBattlefieldFrame:IsShown() then
        MinimapModule._battlefieldWasVisible = true
        MiniMapBattlefieldFrame:Hide()
    end

    if DurabilityFrame and DurabilityFrame:IsShown() then
        MinimapModule._durabilityWasVisible = true
        DurabilityFrame:Hide()
    end
end

local function StartMinimapFade(shouldShow)
    local state = minimapVisibilityState
    local targetAlpha = shouldShow and 1 or 0
    local duration = GetMinimapFadeDuration()

    -- Engine icons can't tween; drop them at the start of a fade-out so they
    -- disappear under cover of the fade. Restored when a fade-in completes.
    if not shouldShow then
        SetEngineIconsVisible(false)
    end
    ...
    if duration <= 0
        ...
        if targetAlpha >= 1 then
            SetEngineIconsVisible(true)
        end
        ApplyMinimapAlpha(targetAlpha)
        return
    end
...
function addon.RefreshMinimapVisibility()
    ...
    RefreshMinimapVisibilityInternal()
end

local minimapFadeFrame = CreateFrame("Frame")
minimapFadeFrame:Show()

minimapFadeFrame:SetScript("OnUpdate", function(_, elapsed)
    local state = minimapVisibilityState
    ...
    ApplyMinimapAlpha(state.alpha)

    -- Bring the engine icon layer back only once a fade-in is fully complete.
    if not state.fading and state.targetAlpha >= 1 then
        SetEngineIconsVisible(true)
    end
end)
...
local minimapHoverElapsed = 0

minimapHoverFrame:SetScript("OnUpdate", function(_, elapsed)
    minimapHoverElapsed = minimapHoverElapsed + elapsed

    if minimapHoverElapsed < 0.05 then
        return
    end

    minimapHoverElapsed = 0

    local config = GetMinimapVisibilityConfig()

    if not config or not config.show_on_hover then
        if minimapVisibilityState.hovered then
            minimapVisibilityState.hovered = false
            RefreshMinimapVisibilityInternal()
        end
        return
    end

    local hovered = IsCursorOverMinimap()

    if hovered ~= minimapVisibilityState.hovered then
        minimapVisibilityState.hovered = hovered
        RefreshMinimapVisibilityInternal()
    end
end)

local minimapVisibilityEvents = CreateFrame("Frame")

minimapVisibilityEvents:RegisterEvent("PLAYER_TARGET_CHANGED")
minimapVisibilityEvents:RegisterEvent("PLAYER_REGEN_DISABLED")
minimapVisibilityEvents:RegisterEvent("PLAYER_REGEN_ENABLED")
minimapVisibilityEvents:RegisterEvent("UNIT_HEALTH")
minimapVisibilityEvents:RegisterEvent("UNIT_MAXHEALTH")
minimapVisibilityEvents:RegisterEvent("UNIT_HEALTH_FREQUENT")
minimapVisibilityEvents:RegisterEvent("UNIT_MANA")
minimapVisibilityEvents:RegisterEvent("UNIT_MAXMANA")
minimapVisibilityEvents:RegisterEvent("UNIT_RAGE")
minimapVisibilityEvents:RegisterEvent("UNIT_FOCUS")
minimapVisibilityEvents:RegisterEvent("UNIT_ENERGY")
minimapVisibilityEvents:RegisterEvent("UNIT_RUNIC_POWER")
minimapVisibilityEvents:RegisterEvent("UNIT_POWER_UPDATE")
minimapVisibilityEvents:RegisterEvent("UNIT_MAXPOWER")
minimapVisibilityEvents:RegisterEvent("UNIT_DISPLAYPOWER")

minimapVisibilityEvents:SetScript("OnEvent", function(_, event, unit)
    if event:sub(1, 5) == "UNIT_" and unit ~= "player" then
        return
    end

    if event == "PLAYER_REGEN_DISABLED" then
        minimapVisibilityState.inCombat = true
    elseif event == "PLAYER_REGEN_ENABLED" then
        minimapVisibilityState.inCombat = false
    end

    RefreshConditionalMinimapFrames()
    RefreshMinimapVisibilityInternal()
end)
