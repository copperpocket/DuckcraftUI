...
local function ReplaceBlizzardFrame(frame)
    ...
    toggleButton:SetScript("OnClick", function(self)
        ...
        if addon.RefreshBuffVisibility then
            addon.RefreshBuffVisibility()
        end
...
-- ============================================================================
-- BUFF FRAME VISIBILITY
-- ============================================================================

local buffVisibilityState = {
    hovered = false,
...
local function GetBuffVisibilityConfig()
    local buffs = addon.db
        and addon.db.profile
        and addon.db.profile.buffs

    return buffs and buffs.visibility
end
...
local function ShouldShowBuffFrame()
    ...
    -- Preserve normal behavior if custom visibility is unused.
    if not config then
        return true
    end
    ...
    if config.show_on_hover and buffVisibilityState.hovered then

        return true
    end

    if config.show_in_combat and buffVisibilityState.inCombat then
        return true
    end
...
local function GetBuffFadeDuration()
    local config = GetBuffVisibilityConfig()
...
local function ApplyBuffFrameAlpha(alpha)
    ...
    for index = 1, 3 do
        ...
        if enchant then
            enchant:SetAlpha(alpha)
        end
    end


    if ConsolidatedBuffs then
        ConsolidatedBuffs:SetAlpha(alpha)
    end
...
local function StartBuffFrameFade(shouldShow)
    local state = buffVisibilityState
    local targetAlpha = shouldShow and 1 or 0
    local duration = GetBuffFadeDuration()
    ...
    if state.fading then
        ...
        currentAlpha = BuffFrame and BuffFrame:GetAlpha() or 1
    end
    ...
    if duration <= 0
        ...
        ApplyBuffFrameAlpha(targetAlpha)
        return
    end
...
function addon.RefreshBuffVisibility()
    ...
    RefreshBuffVisibilityInternal()
end

local buffFadeFrame = CreateFrame("Frame")
buffFadeFrame:Show()

buffFadeFrame:SetScript("OnUpdate", function(_, elapsed)
    local state = buffVisibilityState
    ...
    ApplyBuffFrameAlpha(state.alpha)
end)

local buffHoverFrame = CreateFrame("Frame")
local buffHoverElapsed = 0

buffHoverFrame:SetScript("OnUpdate", function(_, elapsed)
    buffHoverElapsed = buffHoverElapsed + elapsed

    if buffHoverElapsed < 0.05 then
        return
    end
    ...
    if not config or not config.show_on_hover then
        if buffVisibilityState.hovered then
            buffVisibilityState.hovered = false
            RefreshBuffVisibilityInternal()
        end
        return
    end

    local hovered = IsCursorOverBuffFrame()

    if hovered ~= buffVisibilityState.hovered then
        buffVisibilityState.hovered = hovered
        RefreshBuffVisibilityInternal()
    end
end)

local buffVisibilityEvents = CreateFrame("Frame")

buffVisibilityEvents:RegisterEvent("PLAYER_TARGET_CHANGED")
buffVisibilityEvents:RegisterEvent("PLAYER_REGEN_DISABLED")
buffVisibilityEvents:RegisterEvent("PLAYER_REGEN_ENABLED")
buffVisibilityEvents:RegisterEvent("UNIT_HEALTH")
buffVisibilityEvents:RegisterEvent("UNIT_MAXHEALTH")
buffVisibilityEvents:RegisterEvent("UNIT_HEALTH_FREQUENT")
buffVisibilityEvents:RegisterEvent("UNIT_MANA")
buffVisibilityEvents:RegisterEvent("UNIT_MAXMANA")
buffVisibilityEvents:RegisterEvent("UNIT_RAGE")
buffVisibilityEvents:RegisterEvent("UNIT_FOCUS")
buffVisibilityEvents:RegisterEvent("UNIT_ENERGY")
buffVisibilityEvents:RegisterEvent("UNIT_RUNIC_POWER")
buffVisibilityEvents:RegisterEvent("UNIT_POWER_UPDATE")
buffVisibilityEvents:RegisterEvent("UNIT_MAXPOWER")
buffVisibilityEvents:RegisterEvent("UNIT_DISPLAYPOWER")

buffVisibilityEvents:SetScript("OnEvent", function(_, event, unit)
    if event:sub(1, 5) == "UNIT_" and unit ~= "player" then
        return
    end

    if event == "PLAYER_REGEN_DISABLED" then
        buffVisibilityState.inCombat = true
    elseif event == "PLAYER_REGEN_ENABLED" then
        buffVisibilityState.inCombat = false
    end
...
function BuffFrameModule:Enable()
    ...
    if not BuffFrameModule._hookedBuffAnchors then
        ...
        hooksecurefunc("BuffFrame_UpdateAllBuffAnchors", function()
            ...
            -- Blizzard may reset child alpha while rebuilding the layout.
            if addon.RefreshBuffVisibility then
                addon.RefreshBuffVisibility()
            end
        end)
    ...
    if not buffFrame then
        ...
        buffFrame:SetScript("OnEvent", function(self, event, unit)
            if event == "PLAYER_ENTERING_WORLD" then
                ...
                -- Blizzard finishes creating/showing buff elements during
                -- PLAYER_ENTERING_WORLD. Re-apply custom visibility afterward.
                if addon.RefreshBuffVisibility then
                    addon.RefreshBuffVisibility()
                end
    ...
    -- Reapply visibility if Blizzard shows the buff roots after initialization.
    if not BuffFrameModule._visibilityShowHooks then
        BuffFrameModule._visibilityShowHooks = true
        ...
        if ConsolidatedBuffs then
            ConsolidatedBuffs:HookScript("OnShow", function()
                if addon.RefreshBuffVisibility then
                    addon.RefreshBuffVisibility()
                end
            end)
        ...
        if TemporaryEnchantFrame then
            TemporaryEnchantFrame:HookScript("OnShow", function()
                if addon.RefreshBuffVisibility then
                    addon.RefreshBuffVisibility()
                end
            end)
...
function BuffFrameModule:Disable()
    ...
    BuffFrameModule.enabled = false
    BuffFrameModule.initialized = false
