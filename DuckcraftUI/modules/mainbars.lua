...
local function InitializeMainbars()
   ...
   function MainMenuBarMixin:actionbutton_setup()
    ...
    local function RefreshXpRepBars()
        ...
        if addon.RefreshXpRepBarVisibility then
            addon.RefreshXpRepBarVisibility()
        end
    end
   ...
   -- Specific function to disable MainMenuBarMaxLevelBar
    ...
    xpRepEventFrame:RegisterEvent("PLAYER_TARGET_CHANGED")
    xpRepEventFrame:RegisterEvent("PLAYER_REGEN_DISABLED")
    xpRepEventFrame:RegisterEvent("UNIT_HEALTH")
    xpRepEventFrame:RegisterEvent("UNIT_MAXHEALTH")
    xpRepEventFrame:RegisterEvent("UNIT_HEALTH_FREQUENT")
    xpRepEventFrame:RegisterEvent("UNIT_MANA")
    xpRepEventFrame:RegisterEvent("UNIT_MAXMANA")
    xpRepEventFrame:RegisterEvent("UNIT_RAGE")
    xpRepEventFrame:RegisterEvent("UNIT_FOCUS")
    xpRepEventFrame:RegisterEvent("UNIT_ENERGY")
    xpRepEventFrame:RegisterEvent("UNIT_RUNIC_POWER")
    xpRepEventFrame:RegisterEvent("UNIT_POWER_UPDATE")
    xpRepEventFrame:RegisterEvent("UNIT_MAXPOWER")
    xpRepEventFrame:RegisterEvent("UNIT_DISPLAYPOWER")

    xpRepEventFrame:SetScript("OnEvent", function(self, event, unit)
        if not IsModuleEnabled() then
            return
        end
        ...
        if event == "PLAYER_TARGET_CHANGED"
            ...
            or event:sub(1, 5) == "UNIT_" then

            if event == "PLAYER_REGEN_DISABLED" then
                xpRepVisibilityState.xp.inCombat = true
                xpRepVisibilityState.rep.inCombat = true
            elseif event == "PLAYER_REGEN_ENABLED" then
                xpRepVisibilityState.xp.inCombat = false
                xpRepVisibilityState.rep.inCombat = false
            end
            ...
            -- Target/health/power/combat events only need visibility evaluation.
        ...
        -- Re-apply XP/Rep visibility after normal XP, reputation,
        -- level, faction, or combat updates.
        if addon.RefreshXpRepBarVisibility then
            addon.RefreshXpRepBarVisibility()
        end
    end)
...
-- ============================================================================
-- Uses alpha-based visibility so the main bar container remains available for
-- XP, reputation, stance, and related elements.
...
-- Returns true when a secondary bar has visibility behavior enabled.
local function ShouldUseVisibility(barName)
    ...
    return db[barName .. "_show_on_hover"]
        or db[barName .. "_show_in_combat"]
end

-- Deep alpha pass for main-bar art textures.
-- Functional bars and buttons are intentionally excluded.
local function SetMainBarArtAlphaDeep(alpha)
    ...
    local function ShouldSkip(frame)
        ...
        if frame == MainMenuExpBar
            or frame == ReputationWatchStatusBar
            or frame == StanceBarFrame
            or frame == ShapeshiftBarFrame then
            return true
        end

        local name = frame.GetName and frame:GetName() or ""

        if name:find("ActionButton")
            or name:find("MultiBar")
            or name:find("BonusActionButton")
            or name:find("PetActionButton") then
            return true
        end
    ...
    local function ApplyToRegions(frame)
        if not frame or ShouldSkip(frame) then
            return
        end
...
local function GetActionBarFadeDuration()
    local db = addon.db
        and addon.db.profile
        and addon.db.profile.actionbars
...
local function GetMainBarArtBaseAlpha()
    local buttonsConfig = addon.db
        and addon.db.profile
        and addon.db.profile.buttons

    if buttonsConfig and buttonsConfig.hide_main_bar_background then
        return 0
    end
...
local function ApplyMainBarVisibilityAlpha(alpha)
    ...
    -- Main-bar art.
    if addon.pUiMainBarArt then
        addon.pUiMainBarArt:SetAlpha(artAlpha)
    end

    if MainMenuBarArtFrame then
        MainMenuBarArtFrame:SetAlpha(artAlpha)
    end

    if MainMenuBarLeftEndCap then
        MainMenuBarLeftEndCap:SetAlpha(artAlpha)
    end

    if MainMenuBarRightEndCap then
        MainMenuBarRightEndCap:SetAlpha(artAlpha)
    end

    if ActionBarUpButton then
        ActionBarUpButton:SetAlpha(artAlpha)
    end

    if ActionBarDownButton then
        ActionBarDownButton:SetAlpha(artAlpha)
    end

    if MainMenuBarPageNumber then
        MainMenuBarPageNumber:SetAlpha(artAlpha)
    end

    if addon.pUiMainBar then
        if addon.pUiMainBar.BorderArt then
            addon.pUiMainBar.BorderArt:SetAlpha(artAlpha)
        end

        if addon.pUiMainBar.Background then
            addon.pUiMainBar.Background:SetAlpha(artAlpha)
        end
    end

    -- This function is declared above the fade code intentionally.
    SetMainBarArtAlphaDeep(artAlpha)
end

local function ApplyActionBarVisibilityAlpha(barName, frame, alpha)
    if barName == "main" then
        ApplyMainBarVisibilityAlpha(alpha)
        return
    end

    if frame then
        -- Keep the frame shown so hover detection remains available.
        if not InCombatLockdown() then
            frame:Show()
        end
...
local function GetActionBarFrame(barName)
    if barName == "main" then
        return addon.pUiMainBar
    elseif barName == "bottom_left" then
        return MultiBarBottomLeft
    elseif barName == "bottom_right" then
        return MultiBarBottomRight
    elseif barName == "right" then
        return MultiBarRight
    elseif barName == "left" then
        return MultiBarLeft
    end
...
local function StartActionBarFade(barName, frame, shouldShow)
    local fade = actionBarFadeStates[barName]

    if not fade or not frame then
        return
    end

    local targetAlpha = shouldShow and 1 or 0
    local duration = GetActionBarFadeDuration()

    local currentAlpha

    if fade.active then
        currentAlpha = fade.alpha
    elseif barName == "main" then
        local firstButton = _G["ActionButton1"]
        currentAlpha = firstButton and firstButton:GetAlpha() or 1
    else
    ...
    -- Duration zero preserves the original instant behavior.
    if duration <= 0
        or math.abs(currentAlpha - targetAlpha) < 0.001 then

        fade.alpha = targetAlpha
        fade.startAlpha = targetAlpha
        fade.targetAlpha = targetAlpha
        fade.elapsed = 0
        fade.active = false

        ApplyActionBarVisibilityAlpha(barName, frame, targetAlpha)
        return
    end

    fade.alpha = currentAlpha
    fade.startAlpha = currentAlpha
    fade.targetAlpha = targetAlpha
    fade.elapsed = 0
    fade.active = true

    -- Apply immediately so a reversed fade starts from its current alpha.
    ApplyActionBarVisibilityAlpha(barName, frame, currentAlpha)
end

local actionBarFadeFrame = CreateFrame("Frame")
actionBarFadeFrame:Show()

actionBarFadeFrame:SetScript("OnUpdate", function(_, elapsed)
    for barName, fade in pairs(actionBarFadeStates) do
        if fade.active then
            local frame = GetActionBarFrame(barName)
            ...
            if not frame then
                ...
                if duration <= 0 then
                    ...
                    local progress = fade.elapsed / duration
                    ...
                    if progress >= 1 then
                        ...
                        fade.alpha = fade.startAlpha
                            + (fade.targetAlpha - fade.startAlpha) * eased
                    end
                end

                ApplyActionBarVisibilityAlpha(barName, frame, fade.alpha)
            end
        end
    end
end)
...
function addon.UpdateActionBarVisibility(barName, frame)
    if not frame
        ...
        or not addon.db.profile.actionbars then
        return
    end

    -- Vehicle UI manages its own action bars.
...
function addon.RefreshActionBarVisibility()
    ...
    -- Refresh hover state from current cursor position.
    for barName, frame in pairs(barFrames) do
        local state = addon.visibilityStates[barName]

        if state and frame and frame.IsMouseOver then
            state.hovered = frame:IsMouseOver()
        end
    end
...
-- Hover detection with a short debounce.
local hoverTimers = {}
...
local function SetupActionBarHoverDetection(barName, frame)
    ...
    frame:HookScript("OnEnter", function()
        if hoverTimers[barName]
            and addon.core
            and addon.core.CancelTimer then
    ...
    -- Button-level hooks keep hover active across gaps between buttons.
...
local function InitializeActionBarVisibility()
    ...
    -- Poll the actual screen rectangle of the main bar.
    -- This continues to work when the buttons have alpha 0.
    local actionBarHoverPoll = CreateFrame("Frame")
    local hoverPollElapsed = 0

    actionBarHoverPoll:Show()
    actionBarHoverPoll:EnableMouse(false)
    ...
    actionBarHoverPoll:SetScript("OnUpdate", function(_, elapsed)
        hoverPollElapsed = hoverPollElapsed + elapsed

        if hoverPollElapsed < 0.05 then
            return
        end
        ...
        local state = addon.visibilityStates
            and addon.visibilityStates.main
        ...
        -- If hover visibility is disabled, clear any stale hover state.
        if not config.main_show_on_hover then
            if state.hovered then
                state.hovered = false

                if addon.UpdateActionBarVisibility then
                    addon.UpdateActionBarVisibility("main", addon.pUiMainBar)
                end
            end
        ...
        local isHovered = IsCursorOverFrame(addon.pUiMainBar)

        if isHovered ~= state.hovered then
    ...
    combatFrame:RegisterEvent("UNIT_MAXHEALTH")
    combatFrame:RegisterEvent("UNIT_HEALTH_FREQUENT")

    combatFrame:RegisterEvent("UNIT_MANA")
    combatFrame:RegisterEvent("UNIT_MAXMANA")
    combatFrame:RegisterEvent("UNIT_RAGE")
    combatFrame:RegisterEvent("UNIT_FOCUS")
    combatFrame:RegisterEvent("UNIT_ENERGY")
    combatFrame:RegisterEvent("UNIT_RUNIC_POWER")

    -- Keep these for clients that expose the newer event names.
    combatFrame:RegisterEvent("UNIT_POWER_UPDATE")
    combatFrame:RegisterEvent("UNIT_MAXPOWER")
    combatFrame:RegisterEvent("UNIT_DISPLAYPOWER")


    combatFrame:SetScript("OnEvent", function(_, event, unit)
        if event == "PLAYER_REGEN_DISABLED" then
...
function addon.UpdateGryphonStyle()
    ...
    elseif gryphonStyle == "new" then
        MainMenuBarLeftEndCap:SetClearPoint("BOTTOMLEFT", -95, -23)
        MainMenuBarRightEndCap:SetClearPoint("BOTTOMRIGHT", 95, -23)

        if faction == "Alliance" then
            MainMenuBarLeftEndCap:set_atlas(
                "ui-hud-actionbar-gryphon-thick-left",
                true
            )
            MainMenuBarRightEndCap:set_atlas(
                "ui-hud-actionbar-gryphon-thick-right",
                true
            )
        else
            MainMenuBarLeftEndCap:set_atlas(
                "ui-hud-actionbar-wyvern-thick-left",
                true
            )
            MainMenuBarRightEndCap:set_atlas(
                "ui-hud-actionbar-wyvern-thick-right",
                true
            )
        end
    ...
    -- UpdateGryphonStyle() calls Show() on the gryphons. Reapply the
    -- visibility resolver afterward so hidden action-bar art stays hidden.
    if addon.RefreshActionBarVisibility then
        addon.RefreshActionBarVisibility()
    end
end
...
function addon.ApplyAllBarButtonCounts()
    ...
    -- UpdateGryphonStyle() may have shown the gryphons again.
    if addon.RefreshActionBarVisibility then
        addon.RefreshActionBarVisibility()
    end
